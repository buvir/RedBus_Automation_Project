...
# Initialize geocoder
geolocator = Nominatim(user_agent="redbus_state_finder")
...
def get_route_pairs_with_states():
    route_query = "SELECT DISTINCT route_name FROM bus_routes ORDER BY route_name"
    route_df = execute_query(route_query)
    
    route_pairs = []
    for r in route_df['route_name']:
        if ' to ' in r:
            from_city, to_city = r.split(' to ')[0].strip(), r.split(' to ')[1].strip()
            from_state = get_state_from_city(from_city)
            to_state = get_state_from_city(to_city)
            route_pairs.append((from_city, from_state, to_city, to_state))
    return route_pairs

def get_state_from_city(city_name):
    try:
        location = geolocator.geocode(f"{city_name}, India", exactly_one=True, addressdetails=True)
        return location.raw['address'].get('state', 'Unknown') if location else 'Unknown'
    except:
        return 'Unknown'

def get_all_states():
    routes = get_route_pairs_with_states()
    return sorted({state for _, state, _, _ in routes})

def get_cities_for_state(state):
    routes = get_route_pairs_with_states()
    return sorted({city for city, city_state, _, _ in routes if city_state == state})

def get_destinations_for_origin_state(origin_city, origin_state):
    routes = get_route_pairs_with_states()
    return sorted({to_city for from_city, from_state, to_city, _ in routes 
                 if from_city == origin_city and from_state == origin_state})
...
if 'from_state_select' not in st.session_state:
    st.session_state.from_state_select = None
if 'from_city_select' not in st.session_state:
    st.session_state.from_city_select = None
if 'to_state_select' not in st.session_state:
    st.session_state.to_state_select = None
if 'to_city_select' not in st.session_state:
    st.session_state.to_city_select = None
...
from_state = st.sidebar.selectbox(
    "From State", 
    all_states, 
    index=None,
    placeholder="Select departure state",
    key='from_state_select'
)

# Then city selection based on state
from_city = None
if st.session_state.from_state_select:
    cities = get_cities_for_state(st.session_state.from_state_select)
    from_city = st.sidebar.selectbox(
        "From City", 
        cities, 
        index=None,
        placeholder="Select departure city",
        key='from_city_select'
    )

# Then show destination state and city
to_state = None
to_city = None
if st.session_state.from_city_select:
    # Get possible destination states
    routes = get_route_pairs_with_states()
    dest_states = sorted({to_state for from_city, from_state, _, to_state in routes 
                         if from_city == st.session_state.from_city_select and from_state == st.session_state.from_state_select})
    
    to_state = st.sidebar.selectbox(
        "To State", 
        dest_states, 
        index=None,
        placeholder="Select arrival state",
        key='to_state_select'
    )
    
    if st.session_state.to_state_select:
        # Get cities in destination state connected to origin
        destinations = get_destinations_for_origin_state(
            st.session_state.from_city_select,
            st.session_state.from_state_select
        )
        # Filter by selected destination state
        routes = get_route_pairs_with_states()
        valid_dest_cities = {to_city for _, _, to_city, to_state in routes 
                           if to_state == st.session_state.to_state_select}
        filtered_destinations = [city for city in destinations if city in valid_dest_cities]
        
        to_city = st.sidebar.selectbox(
            "To City", 
            sorted(filtered_destinations), 
            index=None,
...
def filter_and_display_buses():
    if st.session_state.from_city_select and st.session_state.to_city_select:
        route_name = f"{st.session_state.from_city_select} to {st.session_state.to_city_select}"
...
if st.session_state.from_city_select and st.session_state.to_city_select:
    ...
    if st.sidebar.button("Search Buses"):
        st.session_state.from_location = f"{st.session_state.from_city_select}, {st.session_state.from_state_select}"
        st.session_state.to_location = f"{st.session_state.to_city_select}, {st.session_state.to_state_select}"
